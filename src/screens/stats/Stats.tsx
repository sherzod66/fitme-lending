...
import { Card, Loader, Text, ThemeProvider } from '@gravity-ui/uikit'
import { MousePointer, TrendingUp, Users } from 'lucide-react'
...
export default function Stats() {
	...
	const sortedPageClicks = [...stat.pageClicks].sort((a, b) => b.clicks - a.clicks)
							...
							<div className='flex items-center gap-4'>
								...
								<div>
									...
									>
										{' '}
										{(
											stat.pageClicks.reduce((sum, page) => sum + page.clicks, 0) / 2
										).toLocaleString()}
									</Text>
								</div>
							</div>
							...
							<div className='mt-4 space-y-2'>
								...
								{sortedPageClicks.length > 5 && (
									<p className='text-gray-500 text-xs italic'>
										...и еще {sortedPageClicks.length - 5} страниц (смотрите таблицу ниже)
									</p>
								)}
							</div>
						</Card>
							...
							<table className='w-full'>
								<thead>
									<tr className='border-b border-gray-700'>
										<th className='text-left py-3 px-4 text-gray-300'>URL страницы</th>
										<th className='text-right py-3 px-4 text-gray-300'>Количество кликов</th>
										<th className='text-right py-3 px-4 text-gray-300'>Процент от общего</th>
									</tr>
								</thead>
								<tbody>
									{stat.pageClicks
										...
										.map((page, index) => {
											const totalClicks = stat.pageClicks.reduce((sum, p) => sum + p.clicks, 0)
											const percentage = ((page.clicks / (totalClicks / 2)) * 100).toFixed(1)
											return (
												<tr
													key={index}
													className='border-b border-gray-800 hover:bg-gray-800/50'
												>
													<td className='py-3 px-4 text-white font-mono text-sm'>{page.url}</td>
